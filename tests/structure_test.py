...
def test_eigenvectors_normalized(xs, ys, zs, masses):
    a_r = nsc_structure.AxisRatios(xs, ys, zs, masses)
    assert np.isclose(np.linalg.norm(a_r.a_vec), 1)
    assert np.isclose(np.linalg.norm(a_r.b_vec), 1)
    assert np.isclose(np.linalg.norm(a_r.c_vec), 1)
