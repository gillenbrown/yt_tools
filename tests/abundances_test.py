...
def test_init_error_checking_length():
    ...
    with pytest.raises(ValueError):
        nsc_abundances.NSC_Abundances([1, 2], [0.2, 0.4],
                                      [0.5, 0.3], [0, 0, 0])
    nsc_abundances.NSC_Abundances([1, 2], [0.2, 0.4], [0.3, 0.3], [0, 0])

def test_init_error_checking_positive_masses():
    """Masses have to be positive"""
    with pytest.raises(ValueError):
        nsc_abundances.NSC_Abundances([-1, 2], [0.1, 0.1], [0.1, 0.1], [0, 0])
...
def test_init_error_checking_z_bounds():
    ...
    with pytest.raises(ValueError):
        nsc_abundances.NSC_Abundances([1, 2], [0.1, 0.1], [-0.1, 0.1], [0, 0])
    with pytest.raises(ValueError):
        nsc_abundances.NSC_Abundances([1, 2], [1.1, 0.1], [0.1, 0.1], [0, 0])
    with pytest.raises(ValueError):
        nsc_abundances.NSC_Abundances([1, 2], [0.1, 0.1], [1.1, 0.1], [0, 0])
    # the sum of the metallicity can't be larger than one, either.
    with pytest.raises(ValueError):
        nsc_abundances.NSC_Abundances([1, 2], [0.6, 0.6], [0.6, 0.2], [0, 0])

def test_init_error_checking_spread_bounds():
    with pytest.raises(ValueError):
        nsc_abundances.NSC_Abundances([1], [0.2], [0.2], [-0.2])

def test_init_value_parsing():
    """I do a few things with the values in the __init__ to be used later."""
    abund = nsc_abundances.NSC_Abundances([1, 2], [0.1, 0.2],
                                          [0.3, 0.4], [0.2, 0.7])
...
def single_mass_zero():
    return nsc_abundances.NSC_Abundances([3] * 4, [0] * 4, [0] * 4, [0] * 4)

@pytest.fixture
def single_mass_one_Ia():
    return nsc_abundances.NSC_Abundances([3] * 4, [1] * 4, [0] * 4, [0] * 4)

@pytest.fixture
def single_mass_one_II():
    return nsc_abundances.NSC_Abundances([3] * 4, [0] * 4, [1] * 4, [0] * 4)

@pytest.fixture
def single_mass_solar_Ia():
    return nsc_abundances.NSC_Abundances([3] * 4, [solar_z] * 4, [0] * 4,
                                         [0] * 4)

@pytest.fixture
def single_mass_solar_II():
    return nsc_abundances.NSC_Abundances([3] * 4, [0] * 4, [solar_z] * 4,
                                         [0] * 4)

@pytest.fixture
def two_star_different_z():
    return nsc_abundances.NSC_Abundances([2, 3], [0.01, 0.02], [0.005, 0.015],
                                         [0, 0])
